...
import { Search, QrCode, FileText, Eye, CheckCircle, Zap } from 'lucide-react';
        ...
        {phase === 'scanning' ? (
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" onClick={handleContinuar}>Ver Expediente</Button>
            </div>
          </>
        )}
      </div>
...
export function ControlFronterizoBusqueda() {
  const navigate = useNavigate();
  const [searchMethod, setSearchMethod] = useState<'qr' | 'run' | 'pasaporte' | 'patente'>('run');
    ...
    if (searchValue) {
      filtered = filtered.filter(tramite => {
        switch (searchMethod) {
          ...
          case 'patente':
            return tramite.vehiculo?.patente.includes(searchValue.toUpperCase());
          default:
            return true;
        }
      });
    ...
    <Layout>
      {showScanner && (
        <ScannerModal
          ...
          onResult={(tramite) => navigate(`/control-fronterizo/${tramite.id}`)}
        />
      )}

      <div className="space-y-6">
        <div>
          <h1 className="text-2xl mb-1">Buscar Trámite</h1>
          <p className="text-muted-foreground">Busque trámites para realizar el control fronterizo</p>
        </div>
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                { key: 'patente', label: 'Buscar por Patente', Icon: FileText },
              ].map(({ key, label, Icon }) => (
                <button
                  key={key}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label htmlFor="searchValue" className="text-base">
                    ...
                    {searchMethod === 'patente' && 'Patente del Vehículo'}
                  </Label>
                  <Button
                    type="button"
                    variant="ghost"
